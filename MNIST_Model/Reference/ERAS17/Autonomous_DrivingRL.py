...
class Actor(nn.Module):
    def __init__(self, sd, ad):
        super().__init__()
        self.l1 = nn.Linear(sd, 400)
        self.l2 = nn.Linear(400, 300)
        self.l3 = nn.Linear(300, ad)
...
class Critic(nn.Module):
    def __init__(self, sd, ad):
        super().__init__()
        self.l1 = nn.Linear(sd + ad, 400)
        self.l2 = nn.Linear(400, 300)
        self.l3 = nn.Linear(300, 1)

        self.l4 = nn.Linear(sd + ad, 400)
...
class CarBrain:
    def __init__(self, map_img):
        ...
        self.input_dim = 10   # 7 sensors + sin + cos + dist
        self.action_dim = 2   # steer, throttle
        self.max_action = torch.tensor([40, 2.0])  # steering, throttle
        
        self.actor = Actor(self.input_dim, self.action_dim)
        self.actor_t = Actor(self.input_dim, self.action_dim)
        self.actor_t.load_state_dict(self.actor.state_dict())

        self.critic = Critic(self.input_dim, self.action_dim)
        self.critic_t = Critic(self.input_dim, self.action_dim)
    ...
    def get_state(self):
        ...
        for a in [-45,-30,-15,0,15,30,45]:
            ...
            for d in range(5, SENSOR_DIST+1, 5):
                x = self.car_pos.x() + math.cos(rad)*d
                y = self.car_pos.y() + math.sin(rad)*d
                if not (0<=x<self.w and 0<=y<self.h):
                    hit = d; break
                c = QColor(self.map.pixel(int(x),int(y)))
                if (c.red()+c.green()+c.blue())/765 < 0.8:
                    hit = d; break
            sensor_vals.append(1.0 - hit/SENSOR_DIST)
        ...
        return np.array(state, np.float32), dist
    ...
    def step(self, action):
        ...
        angle_norm = next_state[7]   # [-1, 1], 0 = facing target
    ...
    # -------------------------
    def optimize(self):
        if len(self.buffer.storage)<BATCH_SIZE: return
        s, ns, a, r, d = self.buffer.sample(BATCH_SIZE)

        with torch.no_grad():
            na = self.actor_t(ns)
            q1t,q2t = self.critic_t(ns, na)
            qt = r + (1-d)*GAMMA*torch.min(q1t,q2t)
        ...
        if self.steps%self.policy_freq==0:
            aloss = -self.critic.Q1(s,self.actor(s)).mean()
    ...
    def finalize_episode(self, episode_reward):
        if not self.alive:
            self.consecutive_crashes = getattr(self, 'consecutive_crashes', 0) + 1
        else:
            self.consecutive_crashes = 0
...
class NeuralNavApp(QMainWindow):
    ...
    def game_loop(self):
        ...
        action = np.zeros(self.brain.action_dim)
        
        prev_target_idx = self.brain.current_target_idx
        
        if self.brain.steps < 1000: # Warmup steps
             action = np.random.uniform(-self.brain.max_action, self.brain.max_action, size=self.brain.action_dim)
        else:
            action = self.brain.select_action(state)
            if self.brain.expl_noise != 0:
                noise = np.random.normal(0, self.brain.expl_noise, size=self.brain.action_dim)
                action = (action + noise)
                action = np.clip(action, -self.brain.max_action, self.brain.max_action)
        ...
        if self.brain.steps % 50 == 0:
            ...
            self.log(f"Step {self.brain.steps}: AngleDiff={angle_diff:.1f}, Turn={action[0]:.2f}, Speed={action[1]:.2f}, Dist={current_dist:.1f}, DistDiff={dist_diff:.2f}")
        ...
        if done:
            ...
            buffer_size = len(self.brain.buffer.storage)
            max_size = self.brain.buffer.max_size
