...
class CarBrain:
    def __init__(self, map_image: QImage):
        ...
        # RL Init - TD3
        self.input_dim = 10  # 7 sensors + sin + cos + distance_to_target
        self.action_dim = 2 # Steering angle, Speed
        self.max_action = torch.tensor([40, 1.0])  # steering, throttle
        ...
        # TD3 Hyperparameters
        self.policy_noise = 4.0  # Noise added to target policy during training
    ...
    def get_state(self):
        ...
        angle_diff = (angle_to_target - self.car_angle + 180) % 360 - 180
        ...
        state = sensor_vals + [angle_sin, angle_cos, norm_dist]
    ...
    def step(self, action):
        ...
        # Observe state BEFORE moving (for curvature)
        ...
        curvature_factor = max(0.2, angle_cos)  # slow down when facing away
        speed = min_speed + throttle * (max_speed - min_speed) * curvature_factor
        ...
        # -----------------------
            ...
            reward = 100.0
            done = not self.switch_to_next_target()
            self.prev_dist = None
            ...
            # -----------------------
            # Alignment reward (smooth, continuous)
            # -----------------------
            reward += max(0.0, angle_cos) * 1.5

            # -----------------------
            # 🔴 WALL AVOIDANCE (KEY FIX)
            # -----------------------
            min_sensor = min(sensors)  # closest obstacle

            # Dense penalty near walls
            if min_sensor > 0.75:
                reward -= (min_sensor - 0.75) * 8.0

            # Extra penalty for being fast near walls
            if min_sensor > 0.75 and speed > 2.5:
                reward -= 2.0

            # -----------------------
            # Control smoothness
            # -----------------------
            reward -= 0.05 * abs(steer)
    ...
    def optimize(self):
        ...
        action = torch.FloatTensor(action).reshape(-1, self.action_dim) # Ensure shape (Batch, Dim)
